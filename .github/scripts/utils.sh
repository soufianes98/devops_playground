...
build_latest_changelog() {
    ...
    while read -r commit; do
        ...
        formatted_item="$(format_changelog_item "$scope" "$message" "$hash" "$USERNAME" "$REPOSITORY_NAME")"
    ...
    for sec_type in "${section_order[@]}"; do
        if [ -n "${sections[$sec_type]}" ]; then
            ...
            final_release_notes+="$clean_content" #Content already includes newlines from format_changelog_item
        fi
    done
